...
static ALWAYS_INLINE void _save_core_context(uint32_t core_id)
{
	...
	z_xtensa_cache_flush(&core_desc[core_id], sizeof(struct core_state));
}
...
void power_gate_entry(uint32_t core_id)
{
	...
	if (core_id == 0) {
		...
		z_xtensa_cache_flush(lpsheader, sizeof(struct lpsram_header));
	...
	z_xtensa_cache_flush(soc_cpus_active, sizeof(soc_cpus_active));
	k_cpu_idle();
